...
const BloodBankDashboard = () => {
  ...
  const isHoursChanged =
    ...
    (operatingHours.open !== (bloodBank.operatingHours.open || "09:00") ||
      ...
      operatingHours.openOnWeekends !==
        (bloodBank.operatingHours.openOnWeekends ?? true) ||
      operatingHours.emergency247 !==
        (bloodBank.operatingHours.emergency247 ?? true));

  const isPreferencesChanged =
    bloodBank &&
    bloodBank.preferences &&
    (preferences.emailNotify !== (bloodBank.preferences.emailNotify ?? true) ||
      ...
      preferences.weeklyReport !==
        (bloodBank.preferences.weeklyReport ?? true) ||
  ...
  useEffect(() => {
    if (profileData) {
      ...
      if (
        bb.location?.coordinates &&
        (bb.location.coordinates[0] !== 0 || bb.location.coordinates[1] !== 0)
      ) {
        setBankLocation({
          type: "Point",
          coordinates: bb.location.coordinates,
        });
      }
    }
  }, [profileData]);
      ...
      async (position) => {
        ...
        try {
          ...
          if (data && data.address) {
            ...
            warning(
              "Location coordinates captured, but address details could not be retrieved. Please enter manually.",
            );
          }
        } catch (err) {
  ...
  const handleApproveRequest = async (requestId) => {
    if (
      !window.confirm("Are you sure you want to approve this blood request?")
    ) {
      return;
    }
  ...
  const handleRejectRequest = async (requestId) => {
    ...
    try {
      await updateRequestStatus({
        ...
        status: "rejected",
        note:
          reason || "Request rejected due to unavailability of blood units.",
      }).unwrap();
  ...
  const handleCreateCamp = async (e) => {
    ...
    try {
      ...
      if (editingCamp) {
        ...
        addNotification(
          `New blood camp "${campForm.name}" created successfully and is now visible to donors!`,
        );
  ...
  const isOutgoingInterBankRequest = (request) => {
    const requesterId = String(
      request?.requestingBloodBank?._id ||
        request?.requestingBloodBank?.id ||
  ...
  const handleTabChange = (tab) => {
    if (inventoryChanged && activeTab === "inventory" && tab !== "inventory") {
      if (
        !window.confirm(
          "You have unsaved inventory changes. Are you sure you want to leave this tab?",
        )
      ) {
        return;
      }
      ...
      if (dataToUse) {
        setInventory(
          dataToUse.map((item) => ({
            ...
            units: item.units || 0,
            status:
              item.units > 10 ? "good" : item.units > 5 ? "low" : "critical",
            lastUpdated: item.lastUpdated,
          })),
        );
      }
  ...
  const renderCampCard = (camp) => {
    const isPastCamp =
      camp.status === "completed" || !isUpcomingDate(camp.date);
    ...
    const progressPercentage = Math.round(
      (registeredCount / targetUnits) * 100,
    );

    return (
      <div
        key={camp._id}
        className={`camp-card event-style-card ${isPastCamp ? "past-camp" : ""}`}
      >
        {/* {isPastCamp && <div className="past-camp-overlay"></div>} */}
        <div className="event-badge camp-badge">Blood Camp</div>
        ...
        <p className="event-description">
          {camp.description || "Blood donation camp organized by blood bank"}
        </p>
        ...
        <div className="event-info-grid">
          ...
          <div className="info-item">
            ...
            <span>
              {camp.startTime || "09:00"} - {camp.endTime || "17:00"}
            </span>
          </div>
          ...
          <div className="info-item">
            ...
            <span>
              {registeredCount} / {camp.targetUnits} donors
            </span>
          </div>
        </div>
        ...
        <div className="camp-contact-info">
          <p>
            <strong>Contact:</strong> {bloodBank?.email || "N/A"} |{" "}
            {bloodBank?.phone || "N/A"}
          </p>
        </div>
          ...
          <div className="progress-bar">
            <div
              ...
              style={{
                ...
                backgroundColor:
                  registeredCount >= targetUnits ? "#b91c1c" : "#dc2626",
              }}
            ></div>
          </div>
    ...
    <div className="blood-bank-dashboard">
      ...
      <main className="dashboard-main">
        ...
        <div className="dashboard-content">
                ...
                <>
                  {loadingRequests ? (
                    ...
                    <div className="empty-state">
                      ...
                      <p>
                        {requestSourceTab === "bank"
                          ? "No pending blood bank requests at the moment"
                          : requestSourceTab === "my"
                            ? "No pending requests sent by your blood bank"
                            : "No pending user blood requests at the moment"}
                      </p>
                    </div>
                  ) : (
                    <div className="requests-list pending-grid">
                      {pendingRequests.map((request) => {
                        const isMyOutgoingRequest =
                        ...
                        return (
                          ...
                          >
                            <div className="request-header">
                              ...
                              {isMyOutgoingRequest ? (
                                <span
                                  className={`request-status-badge ${responseStatus}`}
                                >
                                  {responseStatus}
                                </span>
                              ) : (
                            ...
                            <div className="request-details">
                              {isInterBankRequest(request) ? (
                                isOutgoingInterBankRequest(request) ? (
                                  <>
                                    <p>
                                      <strong>Request Type:</strong> Blood Bank
                                      Transfer
                                    </p>
                                    <p>
                                      <strong>Requested To:</strong>{" "}
                                      {request.targetBloodBank?.name || "N/A"}
                                    </p>
                                    <p>
                                      <strong>Supplier Contact:</strong>{" "}
                                      {request.targetBloodBank?.phone || "N/A"}
                                    </p>
                                    <p>
                                  ...
                                  <>
                                    ...
                                    <p>
                                      ...
                                      {request.requestingBloodBank?.name ||
                                        request.patientName}
                                    </p>
                                    <p>
                                      <strong>Requester Contact:</strong>{" "}
                                      {request.requestingBloodBank?.phone ||
                                        request.contactNumber ||
                                        "N/A"}
                                    </p>
                                    <p>
                                      <strong>Requester Address:</strong>{" "}
                                      {request.requestingBloodBank?.address
                                        ?.street ||
                ...
                <>
                  {loadingApprovedRequests ? (
                    ...
                    <div className="empty-state">
                      ...
                      <p>
                        {requestSourceTab === "bank"
                          ? "No approved blood bank transfer requests yet"
                          : requestSourceTab === "my"
                            ? "No approved requests sent by your blood bank yet"
                            : "You have not approved any user requests yet"}
                      </p>
                    </div>
                  ) : (
                    <div className="requests-list approved-grid">
                      {approvedRequests.map((request) => (
                        <div
                          key={request._id || request.id}
                          className="request-card approved"
                        >
                          <div className="request-header">
                            <span className="blood-type-badge">
                              {request.bloodGroup}
                          ...
                          <div className="request-details">
                            ...
                            <p>
                              <strong>Units Provided:</strong> {request.units}
                            </p>
                            <p>
          ...
          {/* Donations Tab */}
          {activeTab === "donations" && <BloodBankDonations />}

          {/* Events Tab */}
          {activeTab === "events" && <BloodBankEventManager />}
            ...
            <div className="settings-content">
                    ...
                    <div className="photo-preview-wrapper">
                      {photoPreview ? (
                        <img
                          src={photoPreview}
                          alt="Hospital"
                          className="hospital-photo-preview"
                        />
                      ) : (
              ...
              <div className="settings-section">
                ...
                <div className="settings-form">
                  ...
                  (bankLocation.coordinates[0] !== 0 ||
                    bankLocation.coordinates[1] !== 0) ? (
                    <div className="location-info-display">
                      ...
                      <p className="coordinates-display">
                        ...
                        {bankLocation.coordinates[1].toFixed(6)},{" "}
                        {bankLocation.coordinates[0].toFixed(6)}
                      </p>
                    ...
                    <div className="no-location-display">
                      ...
                      <p>
                        No location set. Share your hospital location so
                    ...
                    <label className="checkbox-label" htmlFor="openWeekends">
                      <input
                        ...
                        onChange={(e) =>
                          setOperatingHours({
                            ...operatingHours,
                            openOnWeekends: e.target.checked,
                          })
                        }
                        disabled={updatingProfile}
                      />
              ...
              <div className="settings-section">
                ...
                <div className="settings-form">
                  <p className="settings-description">
                    Export your hospital data and reports for backup or
                    analysis.
                  </p>
      ...
      {showCampDetails && selectedCamp && (
        ...
        >
          <div
            className="modal-content camp-details-modal"
            ...
            <div className="camp-details-content">
              <div className="detail-section">
                ...
                <div className="detail-row">
                  ...
                  <span>
                    {new Date(selectedCamp.date).toLocaleDateString("en-US", {
                      weekday: "long",
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                ...
                <div className="detail-row">
                  ...
                  <span className={`camp-status ${selectedCamp.status}`}>
                    {selectedCamp.status}
                  </span>
                </div>
              </div>
              ...
              <div className="detail-section">
                ...
                <div className="detail-row">
                  ...
                  <span>
                    {selectedCamp.address}, {selectedCamp.city}
                  </span>
                </div>
                {selectedCamp.state && (
                  <div className="detail-row">
              ...
              <div className="detail-section">
                ...
                <div className="progress-bar-large">
                  <div
                    ...
                    style={{
                      width: `${(selectedCamp.collectedUnits / selectedCamp.targetUnits) * 100}%`,
                    }}
                  ></div>
                </div>
                <div className="progress-percentage">
                  {Math.round(
                    (selectedCamp.collectedUnits / selectedCamp.targetUnits) *
                      100,
                  )}
                  % Complete
                </div>
      ...
      {showRegistrations && selectedCamp && (
        ...
        >
          <div
            className="modal-content registrations-modal"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="modal-header">
                      ...
                      {selectedCamp.registeredDonors.map((donor, index) => {
                        ...
                        return (
                          <tr key={index}>
                            ...
                            <td>
                              <span
                                className={`status-badge ${isPastEvent ? (donor.attended ? "attended" : "pending") : "registered"}`}
                              >
                                {isPastEvent
                                  ? donor.attended
                                    ? "ATTENDED"
                        ...
                        return (
                          <>
                            <p>
                              ...
                              {selectedCamp.registeredDonors.length}
                            </p>
                            <p>
                              <strong>Attended:</strong>{" "}
                              {
                                selectedCamp.registeredDonors.filter(
                                  (d) => d.attended,
                                ).length
                              }
                            </p>
