...
const Dashboard = () => {
  ...
  const { data: allRequestsRes, isLoading: loadingAllRequests } =
    useGetAllRequestsQuery({
      status: "pending",
    });

  const { data: myRequestsRes, isLoading: loadingMyRequests } =
    useGetMyRequestsQuery();

  const { data: dashboardStatsRes, isLoading: loadingStats } =
    useGetDashboardStatsQuery();
  ...
  const { data: profileRes, isLoading: loadingProfile } = useGetProfileQuery(
    undefined,
    {
  ...
  const fetchDonorStats = useCallback(() => {
    ...
    const nextStats = {
      totalDonations:
        dashboardStats?.overview?.personalStats?.totalDonations ||
        info.totalDonations ||
        0,
      eventsAttended: registeredEvents,
      bloodDonated:
        dashboardStats?.overview?.personalStats?.totalDonatedVolume ||
        info.totalDonatedVolume ||
        0,
      lastDonation:
        dashboardStats?.overview?.personalStats?.lastDonationDate ||
  ...
  const checkProfileCompletion = useCallback(() => {
    ...
    if (!user.bloodGroup || user.bloodGroup === "")
    ...
    <div className="page-container">
      <div className="dashboard-header">
        ...
        <div className="header-actions">
          <div className="mode-display">
            <span
              className={`mode-badge mode-${user?.activeMode === "donor" ? "donor" : "patient"}`}
            >
          ...
          {user && (
            ...
            >
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                <path
                  d="M14.5 4.5V2.5M14.5 4.5H16.5M14.5 4.5L11.5 7.5M5.5 15.5V17.5M5.5 15.5H3.5M5.5 15.5L8.5 12.5"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
              ...
              Switch to{" "}
              {(user.activeMode || "patient") === "donor" ? "Patient" : "Donor"}
            </button>
          )}
          {(user?.activeMode || "patient") === "donor" && !user?.isDonor && (
            <Link to={ROUTE_PATH.DONOR_FORM} className="btn btn-donor">
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                <path
                  d="M10 2C10 2 5 7.5 5 12C5 14.7614 7.23858 17 10 17C12.7614 17 15 14.7614 15 12C15 7.5 10 2 10 2Z"
                  stroke="currentColor"
                  strokeWidth="2"
                  fill="currentColor"
                  fillOpacity="0.2"
      ...
      {user?.activeMode === "donor" && user?.isDonor ? (
        <div className="donor-mode-container">
          <div className="stats-grid">
            <div className="stat-card">
              <div
                className="stat-icon stat-icon-donations"
                style={{
            ...
            <div className="stat-card">
              <div
                className="stat-icon stat-icon-events"
                style={{
            ...
            <div className="stat-card">
              <div
                className="stat-icon stat-icon-blood"
                style={{
            ...
            <div className="stat-card">
              <div
                className="stat-icon stat-icon-upcoming"
                style={{
            ...
            <div className="stat-card">
              <div
                className="stat-icon stat-icon-camps"
                style={{
          ...
          <div
            className={`eligibility-banner ${isWaitingPeriod ? "status-waiting" : "status-ready"}`}
          >
            <div className="eligibility-info">
              <div
                className="eligibility-icon"
                style={{
                  background: isWaitingPeriod
              ...
              <div className="eligibility-text">
                <h3>
                  {isWaitingPeriod
                    ? `Next Eligibility: ${nextEligibleDate?.toLocaleDateString()}`
                    : "Ready to Save a Life?"}
                </h3>
                <p>
                  {isWaitingPeriod
                    ? "You recently made a donation. For your safety, the system requires a 3-month gap between donations."
                    : "You are eligible to donate! Click the button to start your request."}
                </p>
              </div>
            </div>
            <div className="eligibility-action">
              {isActuallyEligible ? (
                ...
                <button
                  className="btn btn-secondary btn-disabled-eligibility"
                  disabled
            ...
            <div className="donor-info-card">
              ...
              <div className="donor-journey-stats">
                ...
                <div className="journey-item">
                  ...
                  <span className="journey-value">
                    {donorStats.totalDonations * 3}{" "}
                    {donorStats.totalDonations === 0
                      ? "People (Start your journey!)"
                      : "People"}
                  </span>
                </div>
              </div>
            ...
            <div className="activity-section">
              ...
              >
                ...
                <span className="muted-meta">
                  Showing latest {Math.min(myDonations.length, 6)} of{" "}
                  {myDonations.length}
                </span>
              </div>
              {myDonations.length === 0 ? (
                <EmptyState
                  title={
                    <>
                      Welcome to Your Donor Journey! <FaTint color="#e63946" />
                    </>
                  }
                  message="You haven't made any donation requests yet. Start your life-saving journey today!"
                  actionLabel={
                    isActuallyEligible ? "Request to Donate Now" : null
                  }
                  onAction={
                    isActuallyEligible ? () => setShowDonateModal(true) : null
                  }
                />
              ) : (
                <div
                    ...
                    .map((donation) => (
                      ...
                      >
                        ...
                        <div className="request-details">
                          ...
                          {donation.camp && (
                            <p>
                              <strong>Camp:</strong> {donation.camp.name}
                            </p>
                          )}
                          {donation.status === "completed" && (
                            <p>
                              <strong>Volume:</strong>{" "}
                              {getDonationVolumeLiters(donation.volumeDonated) >
                              0
        ...
        <div className="patient-mode-container">
          <div className="stats-grid">
            <div className="stat-card">
              <div
                className="stat-icon stat-icon-requests"
                style={{
                  background: "linear-gradient(135deg, #ef4444, #b91c1c)",
                }}
              >
            ...
            <div className="stat-card">
              <div
                className="stat-icon stat-icon-my"
                style={{
                  background: "linear-gradient(135deg, #3b82f6, #2563eb)",
                }}
              >
                <svg width="32" height="32" viewBox="0 0 20 20" fill="none">
                  ...
                  />
                  <path
                    d="M8 2H8C8 0.895 8.895 0 10 0C11.105 0 12 0.895 12 2H12"
                    stroke="currentColor"
                    strokeWidth="2"
                  />
                  <path
            ...
            <div className="stat-card">
              <div
                className="stat-icon stat-icon-donors"
                style={{
              ...
              >
                <svg width="32" height="32" viewBox="0 0 20 20" fill="none">
                  ...
                  />
                  <path
                    d="M1 17C1 13.686 3.686 11 7 11C10.314 11 13 13.686 13 17"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                  />
                  <path
                    d="M7 17C7 13.686 9.686 11 13 11C16.314 11 19 13.686 19 17"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                  />
          ...
          <div className="eligibility-banner patient-request-banner">
            <div className="eligibility-info">
              ...
              <div className="eligibility-text">
                ...
                <p>
                  Create a blood request to reach out to available donors.
                  Ensure all details are accurate for faster response.
                </p>
              </div>
            </div>
            <div className="eligibility-action">
          ...
          {(() => {
            ...
            const bloodGroupData = Object.entries(bgMap).map(
              ([_id, count]) => ({ _id, count }),
            );
            ...
            const urgencyData = Object.entries(urgMap).map(([_id, count]) => ({
              _id,
              count,
            }));
                  ...
                  <div className="urgency-stats-container">
                    {urgencyData.length > 0 ? (
                      urgencyData.map((item, index) => (
                        <div
                          key={index}
                          className={`urgency-pill ${item._id.toLowerCase()}`}
                        >
          ...
          <div className="activity-section">
            ...
            >
              ...
              <Link to={ROUTE_PATH.CREATE_REQUEST} className="btn btn-outline">
                New Request
              </Link>
            </div>
            {myRequests.length === 0 ? (
                  ...
                  .map((request) => (
                    ...
                    >
                      ...
                      <div className="request-footer">
                        <Link
                          to={ROUTE_PATH.REQUEST_DETAILS.replace(
                            ":requestId",
                            request._id,
                          )}
      ...
      {showDonorModal && (
        <div className="modal-overlay" onClick={() => setShowDonorModal(false)}>
          <div
            className="modal-content donor-modal"
          ...
          >
            ...
            <div className="modal-body">
              <p className="modal-description">
                To switch to donor mode, please confirm your registration as a
                blood donor.
              </p>
              ...
              <div className="checkbox-group">
                <label className="checkbox-label required">
                  ...
                  <span className="checkbox-text">
                    ...
                    <small>
                      I agree to register as a voluntary blood donor
                    </small>
                  </span>
                </label>
              </div>
              ...
              <div className="modal-actions">
                ...
                <button
                  ...
                  disabled={
                    updatingProfile ||
                    togglingMode ||
                    !donorRegistration.registerAsDonor
                  }
                >
                  {updatingProfile || togglingMode
                    ? "Processing..."
                    : "Confirm & Switch"}
                </button>
              </div>
      ...
      {showDonateModal && (
        <DonateBloodModal
          ...
          onSuccess={() => {
            toast.success("Your donation request has been sent for approval.");
            fetchDonorStats();
          }}
        />
      )}
