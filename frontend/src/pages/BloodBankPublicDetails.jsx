...
const BloodBankPublicDetails = () => {
  ...
  if (error || !bank) {
    return (
      <div className="error-container p-20 text-center">
        ...
        <p>The facility you are looking for might have been moved or is no longer listed.</p>
        <button className="btn-primary mt-4" onClick={() => navigate(ROUTE_PATH.BLOOD_BANKS)}>
          Back to Directory
        </button>
      </div>
    );
  ...
  const totalUnits = inventory.reduce((acc, curr) => acc + (curr.units || 0), 0);

  return (
    <div className="bank-details-page">
      <div className="details-shell">
        <header className="details-header-premium">
          <button className="back-circle-btn" onClick={() => navigate(ROUTE_PATH.BLOOD_BANKS)}>
            <FiArrowLeft />
          </button>
          <div className="profile-hero">
            <div className="hero-avatar">
              <img src={bank.profileImage || `https://images.unsplash.com/photo-1519494026892-80bbd2d6fd0d?w=800&q=80`} alt={bank.name} />
            </div>
            <div className="hero-info">
              <div className="title-row">
                <h1>{bank.name}</h1>
                <span className="verified-badge"><FiShield className="icon" /> Government Verified</span>
              </div>
              <p className="subtitle">Licensed Blood Donation & Storage Center</p>
              <div className="hero-pills">
                <span className="hero-pill"><FiCheckCircle /> Open 24/7</span>
                <span className="hero-pill"><FiPackage /> {totalUnits} Units Available</span>
                <span className="hero-pill"><FiActivity /> Active Donor Hub</span>
              </div>
            </div>
          </div>
        </header>
              ...
              <div className="contact-links">
                ...
                <div className="contact-item">
                  ...
                  <div className="text-wrap">
                    ...
                    <p>{bank.address?.street}, {bank.address?.city}, {bank.address?.state}</p>
            ...
            <section className="inventory-card-premium">
              ...
              <div className="inventory-grid-premium">
                {['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-'].map(group => {
                  const stock = inventory.find(i => (i.bloodGroup || i.type) === group);
                  const units = stock?.units || 0;
                  const status = units > 20 ? 'good' : units > 5 ? 'low' : 'critical';

                  return (
                    <div key={group} className={`stock-item-premium ${status}`}>
              ...
              <div className="inventory-footer">
                <p>* Inventory is updated every 30 minutes. Please call the center before visiting for urgent requirements.</p>
