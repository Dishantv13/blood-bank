...
const RequestTimeline = ({ timeline }) => {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case "fulfilled":
        return <FiPackage className="w-4 h-4 text-emerald-500" />;
      case "completed":
  ...
  return (
    <div className="timeline-container">
      <h3 className="text-lg font-bold text-gray-900 mb-6 px-1">
        Request History
      </h3>
        ...
        {timeline.map((event, index) => (
          <div key={index} className="timeline-item">
            ...
            <div className="timeline-content-body">
              ...
              {event.note && <p className="timeline-note-text">{event.note}</p>}
              <span className="timeline-meta-text">
                Updated by{" "}
                {event.updatedByModel === "BloodBank"
                  ? "Blood Bank"
                  : "Requester"}
              </span>
            </div>
