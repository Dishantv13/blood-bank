...
const EventCard = ({ event, isBloodBank = false, onEdit, onDelete, onViewRegistrations, onDetails, onRegister, isRegistered = false }) => {
  ...
  return (
    <div className={`event-card ${isPastEvent ? 'past-event' : ''}`}>
      {/* Event Badge */}
      <div className="event-badge" style={{ backgroundColor: getEventTypeColor(event.eventType) }}>
      ...
      {event.contactInfo && (
        <div className="contact-info">
          <p>
            <strong>Contact:</strong> {event.contactInfo.email} | {event.contactInfo.phone}
          </p>
        </div>
      )}
      ...
      {event.organizedBy && (
        <div className="organizer-info">
          <p>
            <strong>Organized by:</strong> {event.organizedBy.name || event.organizer}
          </p>
        </div>
      )}
      ...
      <div className="event-actions">
        {isBloodBank ? (
          <>
            ...
            <button className="action-btn edit-btn" onClick={onEdit} title="Edit Event"
            ...
            <button className="action-btn delete-btn" onClick={onDelete} title="Delete Event"
              disabled={isPastEvent}
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
