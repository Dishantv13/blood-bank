...
const MatchingRequests = ({ userBloodGroup }) => {
  ...
  const matchingRequests = response?.data?.filter(req => isCompatible(req.bloodGroup)) || [];

  if (isLoading) return <div className="p-4 animate-pulse bg-gray-50 rounded-lg h-32"></div>;
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden group">
      <div className="p-5 border-b border-gray-50 flex items-center justify-between bg-rose-50/30">
        <div>
          ...
          <p className="text-xs text-rose-600 font-medium">Be a lifesaver today. You are a match for these requests!</p>
        </div>
          ...
          <div key={request._id} className="p-5 hover:bg-gray-50 transition-colors">
            <div className="flex justify-between items-start mb-2">
              <div className="flex gap-3">
                ...
                <div>
                  <h4 className="font-bold text-gray-900 leading-none mb-1">{request.patientName}</h4>
            ...
            <div className="flex items-center justify-between mt-4">
              <div className="text-xs text-gray-500">
                Required: <span className="font-bold text-gray-700">{request.units} Units</span>
              </div>
              <Link 
