...
const SignupModal = ({ isOpen, onClose }) => {
  ...
  const getVerificationPayload = (response) => {
    ...
    return {
      ...
      attemptsRemaining:
        data.attemptsRemaining ?? response?.attemptsRemaining ?? 0,
      resendAttemptsRemaining:
        data.resendAttemptsRemaining ?? response?.resendAttemptsRemaining ?? 0,
      resendAvailableInSeconds:
        data.resendAvailableInSeconds ??
        response?.data?.resendAvailableInSeconds ??
        0,
      otpExpiresInSeconds:
        data.otpExpiresInSeconds ?? response?.data?.otpExpiresInSeconds ?? 0,
      maxVerifyAttempts:
        data.maxVerifyAttempts ?? response?.data?.maxVerifyAttempts ?? 5,
    };
  };
    ...
    try {
      ...
      if (result.success) {
        ...
        if (nextState.verificationId) {
          ...
          setOtpError("");
          toast.success(
            `Verification code sent to ${nextState.maskedEmail || "your email"}`,
          );
        } else {
  ...
  const handleResendOtp = async () => {
    if (
      !verificationState?.verificationId ||
      (verificationState.resendAvailableInSeconds || 0) > 0
    )
      return;
    ...
    <div className="signup-modal-otp-wrapper">
      <div className="otp-modal-header">
        <p>
          ...
          <strong>{verificationState.maskedEmail}</strong>
        </p>
      </div>
      ...
      <form onSubmit={handleVerifyOtp} className="signup-modal-form">
        ...
        <div className="form-group">
          ...
          />
          {verificationState.attemptsRemaining > 0 &&
            verificationState.attemptsRemaining <
              verificationState.maxVerifyAttempts && (
            ...
            {...register("address.pincode", {
              setValueAs: (value) =>
                String(value ?? "")
                  ...
                  .slice(0, 6),
              validate: (val) =>
                !val || pincodeValidator(val) === true || pincodeValidator(val),
            })}
            onInput={(event) => {
              event.target.value = String(event.target.value || "")
                .replace(/\D/g, "")
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={verificationState ? "Identify Yourself" : "Create Your Account"}
    >
