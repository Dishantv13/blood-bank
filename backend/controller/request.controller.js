...
export const getAllRequests = asyncHandler(async (req, res) => {
  ...
  successResponse(res, result, HTTPS_CODE.OK_SUCCESS, "All blood requests fetched successfully");
});
...
export const getMyRequests = asyncHandler(async (req, res) => {
  ...
  successResponse(res, result, HTTPS_CODE.OK_SUCCESS, "My blood requests fetched successfully");
});
...
export const createRequest = asyncHandler(async (req, res) => {
  ...
  successResponse(res, result, HTTPS_CODE.CREATED, "Blood request created successfully");
});
...
export const updateRequest = asyncHandler(async (req, res) => {
  ...
  await clearCacheByPrefix("/api/v1/requests");
  successResponse(res, result, HTTPS_CODE.OK_SUCCESS, "Request updated successfully");
});

// Get request details
export const getRequestById = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const viewer = req.bloodBank
    ? {
        type: "bloodbank",
        id: req.bloodBank.bloodBankId || req.bloodBank._id || req.bloodBank.id,
      }
    : req.user
  ...
  const result = await requestService.getRequestById(id, viewer);
  successResponse(
    res,
    result,
    HTTPS_CODE.OK_SUCCESS,
    "Blood request details fetched successfully",
  );
});
...
export const updateRequestStatus = asyncHandler(async (req, res) => {
  ...
  await clearCacheByPrefix("/api/v1/requests");
  successResponse(res, result, HTTPS_CODE.OK_SUCCESS, `Request status updated to ${status}`);
});
...
export const fulfillRequest = asyncHandler(async (req, res) => {
  ...
  const { id } = req.params;
  const bloodBankId =
    req.bloodBank.bloodBankId || req.bloodBank._id || req.bloodBank.id;
  const result = await requestService.fulfillRequest(id, bloodBankId, req.body);
  successResponse(res, result, HTTPS_CODE.OK_SUCCESS, "Blood request fulfilled successfully");
});
...
export const getBloodBankRequests = asyncHandler(async (req, res) => {
  if (!req.bloodBank) {
    return res
      ...
      .json({ success: false, message: "Not authorized as a blood bank" });
  }
  const bloodBankId =
    req.bloodBank.bloodBankId || req.bloodBank._id || req.bloodBank.id;
  const result = await requestService.getBloodBankRequests(
    bloodBankId,
    req.query,
  );
  successResponse(
    ...
    result,
    HTTPS_CODE.OK_SUCCESS,
    "Requests fetched for blood bank successfully",
  );
});
...
export const broadcastRequest = asyncHandler(async (req, res) => {
  ...
  successResponse(res, result, HTTPS_CODE.OK_SUCCESS, result.message);
});
