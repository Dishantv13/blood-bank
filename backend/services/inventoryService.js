...
export const updateInventoryUnits = async (bloodBankId, bloodGroup, units, operation = 'set', session = null) => {
  ...
  const queryFilter = { bloodBank: bloodBankId, 'items.bloodGroup': bloodGroup };
  ...
  if (operation === 'set') {
    ...
    updateOps.$inc = { 'items.$.units': -units };
    // CONCURRENCY FIX: Atomic check in query filter to prevent negative inventory
    queryFilter['items.units'] = { $gte: units };
  } else {
  ...
  if (!updatedInventory) {
    ...
    if (operation === 'subtract') {
      ...
      if (currentStock < units) {
        throw new ApiError(400, `Insufficient ${bloodGroup} units. Available: ${currentStock}, Requested: ${units}`);
      }
    }
...
// Add inventory units
export const addInventoryUnits = async (bloodBankId, bloodGroup, units, session = null) => {
  if (!units || units <= 0) {
...
export const subtractInventoryUnits = async (bloodBankId, bloodGroup, units, session = null) => {
  if (!units || units <= 0) {
    throw new ApiError(400, 'Units must be greater than 0');
  }

  return updateInventoryUnits(bloodBankId, bloodGroup, units, 'subtract', session);
};
