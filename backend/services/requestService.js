...
export const getUserRequests = async (userId, query) => {
  ...
  // Optimized parallel queries
  const [requests, total] = await Promise.all([
    requestRepository.find({ requestedBy: userId, requestType: 'user' }, {
      select: '_id patientName bloodGroup units urgency status createdAt requiredBy hospital',
      sort: { createdAt: -1 },
      skip,
      limit
...
export const getRequestById = async (requestId) => {
  ...
  const request = await requestRepository.findById(requestId, {
    populate: [
      ...
      { path: 'bloodBank', select: 'name email phone address' },
      { path: 'fulfillment.fulfilledBy', select: 'name phone' }
    ]
  });
...
export const getBloodBankRequests = async (bloodBankId, query) => {
  ...
  // Wrap in $and to avoid top-level status overriding the $or clause's status conditions
  if (query.status) {
    filter = { $and: [filter, { status: query.status }] };
  }
...
export const getApprovedRequests = async (bloodBankId, query) => {
  ...
  // Optimized parallel queries
  const [requests, total] = await Promise.all([
    requestRepository.find({ targetBloodBank: bloodBankId, status: 'approved' }, {
    ...
    requestRepository.count({ targetBloodBank: bloodBankId, status: 'approved' })
  ]);
