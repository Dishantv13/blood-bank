...
export const processDonationReminders = async () => {
  ...
  const ninetyOneDaysAgo = new Date(Date.now() - (ELIGIBILITY_DAYS + 1) * MS_PER_DAY);
...
export const checkAndSendSingleReminder = async (userId) => {
  ...
  const lastDonationDate = user.donorInfo?.lastDonationDate || user.lastDonationDate;
  ...
  const alreadyRemindedThisCycle = lastReminderSentAt && new Date(lastReminderSentAt) > new Date(lastDonationDate);
  ...
  if (isEligibleTime && !alreadyRemindedThisCycle) {
    try {
      ...
      console.error(`Failed to send on-demand reminder to ${user.email}:`, error);
    }
  }
};
