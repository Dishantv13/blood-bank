...
export const exportUsers = async () => {
  const users = await userRepository.find({}, { select: serializers.USER_SAFE_FIELDS, limit: exportHelper.MAX_EXPORT_ROWS });
  ...
  return { buffer: await exportHelper.buildWorkbookBuffer('Users', rows), filename: 'users.xlsx', rowsLimited };
};
...
export const exportRequests = async () => {
  ...
  const rows = requests.map((req) => ({
    ...
    RequesterName: exportHelper.csvSafe(req.requestedBy?.name || 'Unknown'),
...
export const exportBloodBanks = async () => {
  const banks = await bloodBankRepository.find(
    {},
    { select: serializers.BLOOD_BANK_SAFE_FIELDS, limit: exportHelper.MAX_EXPORT_ROWS }
  );
...
export const exportCamps = async () => {
  const camps = await bloodCampRepository.find(
    {},
    { populate: { path: 'organizer', select: 'name email phone' }, limit: exportHelper.MAX_EXPORT_ROWS }
  );
  ...
  const rows = camps.map((camp) => ({
    ...
    Organizer: exportHelper.csvSafe(camp.organizerName || camp.organizer?.name || 'Unknown'),
...
export const exportEvents = async () => {
  const events = await eventRepository.find({}, { limit: exportHelper.MAX_EXPORT_ROWS });
...
export const exportUsersCsv = async () => {
  ...
  const buffer = await exportHelper.pipelineCursorToCsv(cursor, transformer);
  return {
    buffer,
...
export const exportRequestsCsv = async () => {
  ...
  const rowsLimited = requests.length === exportHelper.MAX_EXPORT_ROWS;
  const rows = requests.map((req) => ({
    PatientName: exportHelper.csvSafe(req.patientName),
    BloodGroup: req.bloodGroup,
    Units: req.units,
    Hospital: exportHelper.csvSafe(req.hospital?.name || req.hospital?.address || 'N/A'),
    Status: req.status,
    Urgency: req.urgency,
    RequestedAt: new Date(req.createdAt).toLocaleDateString(),
  }));

  const buffer = exportHelper.buildCsvBuffer(rows);
  return {
    buffer,
...
export const exportBloodBanksCsv = async () => {
  ...
  const rows = banks.map((bank) => ({
    ...
    City: exportHelper.csvSafe(bank.address?.city || 'N/A'),
    State: exportHelper.csvSafe(bank.address?.state || 'N/A'),
...
export const exportCampsCsv = async () => {
  ...
  const rowsLimited = camps.length === exportHelper.MAX_EXPORT_ROWS;
  const rows = camps.map((camp) => ({
    Name: exportHelper.csvSafe(camp.name),
    Organizer: exportHelper.csvSafe(camp.organizerName || 'N/A'),
    Venue: exportHelper.csvSafe(camp.venue || 'N/A'),
    City: exportHelper.csvSafe(camp.city || 'N/A'),
    Date: camp.date ? new Date(camp.date).toLocaleDateString() : 'N/A',
    Status: camp.status,
  }));
...
export const exportEventsCsv = async () => {
  ...
  const rows = events.map((event) => ({
    ...
    Date: event.date ? new Date(event.date).toLocaleDateString() : 'N/A',
    Location: exportHelper.csvSafe(event.location?.name || event.location?.address || 'N/A'),
    Active: event.isActive ? 'Yes' : 'No',
  }));
...
// All-in-One Export
export const exportAllData = async (format = 'xlsx', res) => {
  const getCursors = () => ({
    users: userRepository.model.find({}).select('name email phone bloodGroup isAvailable isDonor createdAt').cursor(),
    requests: requestRepository.model.find({}).select('patientName bloodGroup units hospital status urgency createdAt').cursor(),
    banks: bloodBankRepository.model.find({}).select('name email phone licenseNumber address approvalStatus createdAt').cursor(),
    camps: bloodCampRepository.model.find({}).select('name organizerName venue city date status createdAt').cursor(),
    events: eventRepository.model.find({}).select('title eventType date location isActive createdAt').cursor(),
  });

  if (format === 'csv') {
    const cursors = getCursors();
    res.write('Collection,Col1,Col2,Col3,Col4,Col5,Col6,Col7,Col8\n');
    
    const writeRow = (arr) => {
      const line = arr.map(val => `"${String(val ?? '').replace(/"/g, '""')}"`).join(',') + '\n';
      res.write(line);
    };

    for await (const u of cursors.users) {
      writeRow(['Users', exportHelper.csvSafe(u.name), exportHelper.csvSafe(u.email), u.phone || 'N/A', u.bloodGroup || 'N/A', u.isAvailable ? 'active' : 'inactive', u.isDonor ? 'Yes' : 'No', new Date(u.createdAt).toLocaleDateString()]);
    }
    for await (const r of cursors.requests) {
      writeRow(['Requests', exportHelper.csvSafe(r.patientName), r.bloodGroup, r.units, exportHelper.csvSafe(r.hospital?.name || r.hospital?.address || 'N/A'), r.status, r.urgency, new Date(r.createdAt).toLocaleDateString()]);
    }
    for await (const b of cursors.banks) {
      writeRow(['BloodBanks', exportHelper.csvSafe(b.name), exportHelper.csvSafe(b.email), b.phone || 'N/A', b.licenseNumber || 'N/A', exportHelper.csvSafe(b.address?.city || 'N/A'), exportHelper.csvSafe(b.address?.state || 'N/A'), b.approvalStatus || 'pending', new Date(b.createdAt).toLocaleDateString()]);
    }
    for await (const c of cursors.camps) {
      writeRow(['Camps', exportHelper.csvSafe(c.name), exportHelper.csvSafe(c.organizerName || 'N/A'), exportHelper.csvSafe(c.venue || 'N/A'), exportHelper.csvSafe(c.city || 'N/A'), c.date ? new Date(c.date).toLocaleDateString() : 'N/A', c.status]);
    }
    for await (const e of cursors.events) {
      writeRow(['Events', exportHelper.csvSafe(e.title), e.eventType || 'N/A', e.date ? new Date(e.date).toLocaleDateString() : 'N/A', exportHelper.csvSafe(e.location?.name || e.location?.address || 'N/A'), e.isActive ? 'Yes' : 'No']);
    }
    res.end();
    return;
  }
  ...
  userSheet.addRow(['Name', 'Email', 'Phone', 'BloodGroup', 'Status', 'IsDonor', 'CreatedAt']).commit();
  for await (const u of cursors.users) {
    userSheet.addRow([exportHelper.csvSafe(u.name), exportHelper.csvSafe(u.email), u.phone || 'N/A', u.bloodGroup || 'N/A', u.isAvailable ? 'active' : 'inactive', u.isDonor ? 'Yes' : 'No', new Date(u.createdAt).toLocaleDateString()]).commit();
  }
  userSheet.commit();

  const requestSheet = workbook.addWorksheet('Requests');
  requestSheet.addRow(['PatientName', 'BloodGroup', 'Units', 'Hospital', 'Status', 'Urgency', 'RequestedAt']).commit();
  for await (const r of cursors.requests) {
    requestSheet.addRow([exportHelper.csvSafe(r.patientName), r.bloodGroup, r.units, exportHelper.csvSafe(r.hospital?.name || r.hospital?.address || 'N/A'), r.status, r.urgency, new Date(r.createdAt).toLocaleDateString()]).commit();
  }
  requestSheet.commit();

  const bankSheet = workbook.addWorksheet('BloodBanks');
  bankSheet.addRow(['Name', 'Email', 'Phone', 'LicenseNumber', 'City', 'State', 'Status', 'CreatedAt']).commit();
  for await (const b of cursors.banks) {
    bankSheet.addRow([exportHelper.csvSafe(b.name), exportHelper.csvSafe(b.email), b.phone || 'N/A', b.licenseNumber || 'N/A', exportHelper.csvSafe(b.address?.city || 'N/A'), exportHelper.csvSafe(b.address?.state || 'N/A'), b.approvalStatus || 'pending', new Date(b.createdAt).toLocaleDateString()]).commit();
  }
  bankSheet.commit();

  const campSheet = workbook.addWorksheet('Camps');
  campSheet.addRow(['Name', 'Organizer', 'Venue', 'City', 'Date', 'Status']).commit();
  for await (const c of cursors.camps) {
    campSheet.addRow([exportHelper.csvSafe(c.name), exportHelper.csvSafe(c.organizerName || 'N/A'), exportHelper.csvSafe(c.venue || 'N/A'), exportHelper.csvSafe(c.city || 'N/A'), c.date ? new Date(c.date).toLocaleDateString() : 'N/A', c.status]).commit();
  }
  campSheet.commit();

  const eventSheet = workbook.addWorksheet('Events');
  eventSheet.addRow(['Title', 'EventType', 'Date', 'Location', 'Active']).commit();
  for await (const e of cursors.events) {
    eventSheet.addRow([exportHelper.csvSafe(e.title), e.eventType || 'N/A', e.date ? new Date(e.date).toLocaleDateString() : 'N/A', exportHelper.csvSafe(e.location?.name || e.location?.address || 'N/A'), e.isActive ? 'Yes' : 'No']).commit();
  }
  eventSheet.commit();
...
export const getAllUsers = async (page = 1, limit = 10, filters = {}) => {
  ...
  if (filters.search && typeof filters.search === 'string') {
    const escapedSearch = filters.search.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').substring(0, 100);
    query.$or = [
  ...
  const result = await userRepository.getAllUsersPaginated({ query, skip, limit: safeLimit });
...
export const getUserById = async (userId) => {
  const user = await userRepository.findById(userId, { select: serializers.USER_SAFE_FIELDS, lean: true });
...
export const getAllBloodBanks = async (page = 1, limit = 10, filters = {}) => {
  ...
  const result = await bloodBankRepository.getAllBloodBanksPaginated({ query, skip, limit: safeLimit });
...
export const getBloodBankById = async (bankId) => {
  ...
  const [bank, inventory] = await Promise.all([
    bloodBankRepository.findById(bankId, { select: serializers.BLOOD_BANK_SAFE_FIELDS, lean: true }),
...
export const updateBloodBankStatus = async (bankId, status, reviewContext = {}) => {
  ...
  const reviewer =
    reviewContext.reviewedBy || reviewContext.adminEmail || reviewContext.adminName || 'admin';
  ...
  if (status === 'rejected') {
    ...
    await bloodBankService.sendBloodBankRegistrationRejectedEmail(bank, rejectionReason);
...
export const getAllCamps = async (page = 1, limit = 10, filters = {}) => {
  ...
  const result = await bloodCampRepository.getAllCampsPaginated({ query, skip, limit: safeLimit });
...
export const getAllEvents = async (page = 1, limit = 10, filters = {}) => {
  ...
  const result = await eventRepository.getAllEventsPaginated({ query, skip, limit: safeLimit });
...
export const getAllRequests = async (page = 1, limit = 10, filters = {}) => {
  ...
  if (filters.requestType && ['user', 'bloodbank'].includes(filters.requestType))
  ...
  const result = await requestRepository.getAllRequestsPaginated({ query, skip, limit: safeLimit });
...
export const getAllDonations = async (page = 1, limit = 10, filters = {}) => {
  ...
  const result = await donationRepository.getAllDonationsPaginated({ matchStage, skip, limit: safeLimit });
...
export const getInventoryOverview = async (page = 1, limit = 10, filters = {}) => {
  ...
  const matchStage = {};
  if (filters.search && typeof filters.search === 'string') {
    matchStage.bloodBankName = { $regex: escapeRegex(filters.search), $options: 'i' };
  }
  ...
  const stats = {
    ...
    byBloodType: rawStats.reduce((acc, s) => ({ ...acc, [s._id]: s.total }), {}),
    expiringSoon: 0,
    expired: 0,
  };
...
export const invalidateDashboardStats = () => cacheManager.del('admin:dashboard_stats');
...
export const getDashboardStats = async () => {
  ...
  const totalBloodInventory = Object.values(bloodTypeDistribution).reduce((a, b) => a + b, 0);
