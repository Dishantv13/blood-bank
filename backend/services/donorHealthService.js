...
export const submitHealthForm = async (userId, data) => {
  ...
  const existing = await DonorHealth.findOne({ donor: userId, status: 'pending' }).lean();
  if (existing) {
    throw new ApiError(400, 'You already have a pending health form. Please wait for review.');
  }
...
export const getLatestForm = async (userId) => {
  const form = await DonorHealth.findOne({ donor: userId }).sort({ submittedAt: -1 }).lean();
...
export const checkEligibility = async (userId) => {
  const form = await DonorHealth.findOne({ donor: userId }).sort({ submittedAt: -1 }).lean();
  if (!form) return { hasForm: false, message: 'Please complete the health form first' };
...
export const getFormById = async (id) => {
  const form = await DonorHealth.findById(id).populate('donor', 'name email phone').lean();
...
export const updateForm = async (id, userId, data) => {
  const form = await DonorHealth.findById(id);
  if (!form) throw new ApiError(404, 'Health form not found');
  if (form.donor.toString() !== userId.toString()) throw new ApiError(403, 'Not authorized');
  if (!['pending', 'requires_review'].includes(form.status)) {
    throw new ApiError(400, 'Cannot update a form that has been approved or rejected');
  }

  const updateFields = [
    'fullName', 'dateOfBirth', 'gender', 'bloodGroup', 'weight', 'phone', 'email', 'address', 'city',
    'medicalConditions', 'recentActivities', 'currentHealth', 'lifestyle', 'donationHistory', 'consent'
  ];
