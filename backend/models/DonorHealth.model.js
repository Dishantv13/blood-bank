...
DonorHealthSchema.pre("save", async function () {
  ...
  if (this.medicalConditions.hepatitisBC)
    ineligibleConditions.push("Hepatitis B/C");
  if (this.medicalConditions.cancer)
    ineligibleConditions.push("Cancer history");
  if (this.medicalConditions.heartDisease)
    ineligibleConditions.push("Heart disease");
  if (this.medicalConditions.bleedingDisorder)
    ineligibleConditions.push("Bleeding disorder");

  // Check recent activities
  if (this.recentActivities.tattooOrPiercing)
    ineligibleConditions.push("Recent tattoo/piercing (wait 6 months)");
  if (this.recentActivities.travelToMalariaArea)
    ineligibleConditions.push("Travel to malaria-endemic area (wait 3 months)");

  // Check current health
  if (this.currentHealth.recentFeverOrIllness)
    ineligibleConditions.push("Recent fever or illness");
  if (this.currentHealth.recentAlcoholConsumption)
    ineligibleConditions.push("Recent alcohol consumption (wait 24 hours)");
